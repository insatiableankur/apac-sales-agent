...
// ─── SYSTEM PROMPT ────────────────────────────────────────────────────────────
const buildSystemPrompt = () => `You are the APAC Enterprise SaaS Sales Intelligence Engine — an AI trained on the complete methodologies of Ankur Sehgal: 15 years closing enterprise deals across APAC & MEA, 7x President's Club, $25M ARR builder at Salesforce (Slack), Workiva, and PeopleStrong. You have closed landmark deals with Petronas, Axiata Group, OCBC, BDO Unibank, Sephora SEA, HCL Technologies, and others.

When given a company and deal context, you will produce a comprehensive, STRUCTURED JSON intelligence brief covering all 7 modules. Your output must be only valid JSON — no markdown, no explanation, no preamble.
...

{
  "accountBrief": {
    "icpScore": 87,
    "icpRating": "Strong Fit",
    "tier": "Tier 1 — Strategic",
    "dealPotential": "$200K–$400K ACV",
    "companyProfile": "2-3 sentence company snapshot with size, market position, and relevance to the product being sold.",
    "whyNow": "2-3 sentence explanation of why this is the RIGHT TIME to engage — regulatory, competitive, macro, or internal triggers.",
    "keyTriggers": ["Trigger event 1 — specific and research-backed","Trigger event 2","Trigger event 3"],
    "painPoints": [
      {"pain": "Primary pain description", "businessImpact": "Quantified or qualified business impact", "urgency": "high"},
      {"pain": "Secondary pain", "businessImpact": "Impact description", "urgency": "medium"},
      {"pain": "Tertiary pain", "businessImpact": "Impact description", "urgency": "low"}
    ],
    "buyingCulture": "1-2 sentences on how this specific company in this specific market makes buying decisions — hierarchy, relationship needs, procurement process.",
    "apacMarketContext": "1-2 sentences on broader market context — regulatory environment, competitive landscape, macro trends relevant to this sale."
  },
  "meddpicc": {
    "overallHealth": "amber",
    "forecastCategory": "Pipeline",
    "elements": {
      "metrics": {"status": "red", "label": "Metrics", "description": "What we know and don't know about their success metrics", "evidence": "Specific evidence or gap description", "nextAction": "Specific action to move from red/amber to green", "questions": ["Question 1 to uncover metrics","Question 2"]},
      "economicBuyer": {"status": "amber", "label": "Economic Buyer", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]},
      "decisionCriteria": {"status": "green", "label": "Decision Criteria", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]},
      "decisionProcess": {"status": "red", "label": "Decision Process", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]},
      "paperProcess": {"status": "red", "label": "Paper Process", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]},
      "identifiedPain": {"status": "green", "label": "Identified Pain", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]},
      "champion": {"status": "amber", "label": "Champion", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]},
      "competition": {"status": "amber", "label": "Competition", "description": "...", "evidence": "...", "nextAction": "...", "questions": ["...","..."]}
    },
    "dealRisks": ["Top risk 1 specific to this deal", "Risk 2", "Risk 3"],
    "winConditions": ["What must be true for this deal to close", "Win condition 2", "Win condition 3"]
  },
  "stakeholders": {
    "buyingCommittee": [
      {
        ...
        "priority": 1,
        "accessStatus": "not-engaged",
        "motivations": "What this person cares about professionally and politically",
        "fears": "What they're afraid of — career risk, audit risk, budget scrutiny",
        "engagementStrategy": "Specific approach to get and keep this person's attention",
        "talkTrack": "2-3 sentence opener or value proposition tailored to this exact role",
        "warningSign": "Red flag that signals this person is becoming a blocker"
      },
      {
        "role": "Head of Finance Transformation",
        "archetype": "Champion",
        "priority": 2,
        "accessStatus": "engaged",
        "motivations": "...",
        "fears": "...",
        "engagementStrategy": "...",
        "talkTrack": "...",
      ...
      },
      {
        ...
        "archetype": "Procurement / Gatekeeper",
        "priority": 4,
    ...
    "championDevelopmentScore": 45,
    "championGaps": ["Gap 1 in current champion relationship", "Gap 2", "Gap 3"],
    "multithreadingStatus": "At risk — likely single-threaded. Recommend immediate action to build 2 additional relationships."
  },
  "outreach": {
    "coldEmail": {
      "subject": "Specific, personalised subject line for the economic buyer",
      "preheader": "Preview text that complements the subject",
      "body": "Full email body with [PERSONALISE] placeholders where needed. 5-element Ankur framework: Personal Hook → Peer Frame → Insight Drop → Soft Ask → Social Proof. 150-200 words max. Professional but human. APAC-appropriate tone.",
      "followUp1": {
        "dayToSend": 5,
        "subject": "Follow-up subject line",
        "body": "Short 3-sentence follow-up referencing first email"
      },
      "followUp2": {
        "dayToSend": 12,
        "subject": "Final follow-up subject",
        "body": "Break-up style follow-up — give them an easy out while keeping the door open"
      }
    },
  ...
  "discoveryQuestions": {
    "openingFramer": "1-2 sentence agenda setter for the discovery call that signals expertise without pitching",
    "categories": [
      {
        "category": "Current State & Pain",
        "questions": [
          {"question": "Full discovery question text", "intent": "What you're trying to uncover", "followUp": "Follow-up if they give a vague answer"},
      ...
      {
        "category": "Decision Process & Stakeholders",
        "questions": [
          {"question": "...", "intent": "...", "followUp": "..."},
          {"question": "...", "intent": "...", "followUp": "..."},
          {"question": "...", "intent": "...", "followUp": "..."}
      ...
      {
        "category": "Urgency & Compelling Event",
        "questions": [
          {"question": "...", "intent": "...", "followUp": "..."},
          {"question": "...", "intent": "...", "followUp": "..."}
        ]
    ...
    "redFlags": ["Red flag answer to watch for 1", "Red flag 2", "Red flag 3"],
    "idealCallOutcome": "What a perfect discovery call ending looks like for this specific deal"
  },
  "commandOfMessage": {
    "salesStage": "Evaluation",
    "stageRationale": "Why this deal is at this stage based on the information provided",
    "beforeScenario": "What life looks like for this prospect TODAY without the solution — specific to their company and industry",
    "afterScenario": "What success looks like 12 months after going live — tangible, quantified outcomes",
    "requiredCapabilities": [
      {"capability": "Specific capability name", "proofPoint": "How your product delivers this — concrete example or case study reference"},
      {"capability": "...", "proofPoint": "..."},
      {"capability": "...", "proofPoint": "..."}
    ],
    "uniqueDifferentiators": ["Differentiator 1 specific to this competitive context", "Differentiator 2", "Differentiator 3"],
    "valueDrivers": [
      {"driver": "Cost Reduction", "specifics": "Specific cost reduction opportunity for this company", "estimatedImpact": "$X or X%"},
      {"driver": "Risk Mitigation", "specifics": "Specific risk being mitigated", "estimatedImpact": "Description"},
      {"driver": "Revenue / Growth", "specifics": "Growth opportunity enabled", "estimatedImpact": "$X or X%"}
    ],
    "objectionHandlers": [
      {"objection": "Most likely objection for this deal/company/stage", "response": "How to handle it — Ankur style: acknowledge, reframe, evidence"},
      {"objection": "...", "response": "..."},
      {"objection": "...", "response": "..."}
    ],
    "closingHypothesis": "The single most compelling reason this company should buy — one sentence, boardroom-ready"
  },
  "nextBestActions": [
...
export default function APACSalesAgent() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      try {
        setAnalyzeStep(0);
        liveIntel = ""; // web search disabled temporarily
      } catch(e) {}
