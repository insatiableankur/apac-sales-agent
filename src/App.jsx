...
CRITICAL STAKEHOLDER RULES (non-negotiable):
- NEVER cite named customers (e.g. ServiceNow, Databricks, Adobe, Snowflake, JPMorgan) in talkTrack, motivations, fears, or engagementStrategy. If a proof point would strengthen a talkTrack, insert the literal string [REP TO FILL - INSERT CUSTOMER PROOF POINT].
- NEVER invent statistics: no percentages (e.g. "75% reduction"), no time deltas (e.g. "from 8 weeks to 3 weeks"), no dollar figures not already present in the user message.
- FORBIDDEN phrases in talkTrack and engagementStrategy: "companies like yours", "similar organizations have seen", "our customers typically", "peer enterprises", "we have helped companies like".
- NEVER use a specific vendor name for the SELLER. Never say "Workiva", "SAP", "Oracle", "Salesforce" etc. as the seller. Refer to the seller as "our platform" or "we". Seller identity is generic.
- Generic industry framing without numbers is acceptable. A placeholder is credible; an invented stat is disqualifying.
`;
...
{"outreach":{"coldEmail":{"subject":"...","preheader":"...","body":"150-200 word email","followUp1":{"dayToSend":5,"subject":"...","body":"..."},"followUp2":{"dayToSend":12,"subject":"...","body":"..."}},"linkedInMessage":"under 300 chars","executiveReferral":"...","sendingTips":["..."]},"discoveryQuestions":{"openingFramer":"...","categories":[{"category":"Current State","questions":[{"question":"...","intent":"...","followUp":"..."}]}],"redFlags":["..."],"idealCallOutcome":"..."},"commandOfMessage":{"salesStage":"Evaluation","stageRationale":"...","beforeScenario":"...","afterScenario":"...","requiredCapabilities":[{"capability":"...","proofPoint":"..."}],"uniqueDifferentiators":["..."],"valueDrivers":[{"driver":"Cost Reduction","specifics":"...","estimatedImpact":"~Nx annual investment OR ~N% of annual contract OR [REP TO FILL — QUANTIFY WITH FINANCE]"}],"objectionHandlers":[{"objection":"...","response":"..."}],"closingHypothesis":"..."},"nextBestActions":[{"priority":1,"action":"...","why":"...","timeframe":"This week"},{"priority":2,"action":"...","why":"...","timeframe":"Next 2 weeks"},{"priority":3,"action":"...","why":"...","timeframe":"This month"}]}

CRITICAL VALUE DRIVER RULES (non-negotiable):
- The estimatedImpact field MUST be expressed as ONE of: (a) a multiplier of annual deal size (e.g. "~1.2x annual investment"), (b) a percentage of annual contract value (e.g. "~60% of annual contract"), (c) qualitative framing (e.g. "Supports FY26 compliance deadline"), OR (d) the placeholder "[REP TO FILL — QUANTIFY WITH FINANCE]".
- NEVER invent absolute dollar figures in estimatedImpact. The only exception: if ROI data is provided in the user message, you may reference figures drawn directly from that ROI data.
- FORBIDDEN substrings in estimatedImpact: "market cap", "market value", "brand value", "stock price", "share price", "valuation increase", "investor confidence worth", "$X in market".
- CAP: multipliers must not exceed 5x annual investment. A higher number is not credible to a senior CFO and will be rejected as AI theatre.
- World-class rule: a defensible multiplier beats an invented absolute. When in doubt, use the placeholder.

CRITICAL OUTREACH RULES (non-negotiable):
- NEVER cite named customers (e.g. ServiceNow, Adobe, Databricks, Snowflake) in coldEmail, followUp1, followUp2, linkedInMessage, executiveReferral, or sendingTips. Use [REP TO FILL - INSERT CUSTOMER PROOF POINT] instead.
- NEVER invent statistics: no percentages ("40-60%", "75% reduction"), no time deltas ("from 8 weeks to 3 weeks", "12 to 5 days"), no dollar figures not already in the user message.
- NEVER use a specific vendor name for the SELLER in any email body, subject line, or message. Refer to "our platform" or "we". Never name "Workiva", "Salesforce", "SAP", "Oracle" etc. as the selling vendor.
...
CRITICAL CoM RULES (non-negotiable):
- closingHypothesis: NEVER use a specific vendor name for the SELLER. Do not say "Workiva", "SAP", "Oracle", "Salesforce" etc. as the selling vendor. Refer to "our platform" or the generic solution category.
- objectionHandlers: NEVER invent customer peer benchmarks ("our customers typically see 60% reduction", "similar banks have cut close times in half"). No fabricated ROI claims ("pays for itself within the first year"). If a quantitative claim would strengthen a response, use [REP TO FILL - INSERT CUSTOMER PROOF POINT] instead.
- FORBIDDEN phrases in objectionHandlers and closingHypothesis: "our customers typically", "companies like yours", "similar organizations have seen", "peer enterprises", "we typically see", "typically pays for itself", "we have helped companies like".
...
CRITICAL POV RULES (non-negotiable):
- Stay anchored to the product category implied by the seller's product field. If the product is a Financial Reporting and ESG Platform, the POV is about finance and ESG - NOT HR tech, NOT martech, NOT any other category. Detect and prevent industry drift.
- For trends with data points: only cite a specific statistic if web search returned a verifiable source for it. Otherwise, frame qualitatively ("adoption is accelerating", "regulatory pressure is intensifying") without inventing numbers.
- For analystPerspective: do NOT fabricate what Gartner, Forrester, or IDC said. If no real analyst citation is available from the accompanying research, use the literal string [REP TO FILL - CITE ANALYST SOURCE].
- For peersAreMoving: do NOT name specific peer customers (e.g. "ServiceNow implemented..."). Use anonymous framing ("several global enterprises are moving toward...") or [REP TO FILL - CITE PEER EXAMPLE].
- NEVER use a specific vendor name for the SELLER. Refer to "our platform" or the generic solution category.
- closingPerspective must position the seller as a trusted advisor, not a named brand. No specific vendor self-promotion.
`,
                  ...
                  <div className="inline-section">
                    ...
                    <button onClick={async () => {
                      ...
                      try {
                        const res = await fetch("/api/anthropic", { method:"POST", headers:{"Content-Type":"application/json"},
                          body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:3000, stream:true,
                            system:`You are the Global Enterprise SaaS Sales Intelligence Engine. Return ONLY valid JSON matching this exact schema: {"meetingObjective":"Specific outcome required from this meeting — ONE sentence","personIntel":{"archetype":"Champion|Economic Buyer|Technical Evaluator|User|Gatekeeper","likelyMotivations":["motivation rooted in this role — NOT an invented statistic"],"whatTheyWillJudgeYouOn":"What this persona typically evaluates sellers on — generic industry framing only"},"openingLine":"Conversation opener referencing their PUBLIC context (role, industry, stated priorities). NEVER cite named customer stories or invented statistics. If a proof point would strengthen the line, insert the literal string [REP TO FILL — INSERT CUSTOMER PROOF POINT]","powerQuestions":[{"question":"Open discovery question","intent":"What you aim to uncover","expectedInsight":"Type of answer you expect — describe the shape, NOT a fabricated number"},{"question":"...","intent":"...","expectedInsight":"..."},{"question":"...","intent":"...","expectedInsight":"..."}],"landminesToAvoid":["Assumption or behaviour to avoid — generic pattern","...","..."],"successCriteria":"What good looks like by end of meeting — observable behaviours","followUpTemplate":"Draft follow-up. Use [REP TO FILL — INSERT CUSTOMER PROOF POINT] where a named reference would belong."} CRITICAL MEETING PREP RULES (non-negotiable): (1) NEVER cite named customers (e.g. Databricks, Snowflake, a Fortune 500 bank) unless the name appears in the user message — use [REP TO FILL — INSERT CUSTOMER PROOF POINT] instead. (2) NEVER invent statistics: no percentages like "40% reduction", no time deltas like "cut from 12 to 5 days", no dollar figures not already in the user message. (3) BANNED phrases: "companies like yours", "our customers typically", "customers have reduced", "similar companies have seen", "peer organizations", any variant that implies a quantified peer benchmark. (4) Generic industry framing is OK without a number ("finance teams often prioritize close-time reduction" YES; "finance teams typically see 40% reduction" NO). (5) A placeholder is credible; an invented stat is disqualifying. When in doubt, use the placeholder.`,
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch("/api/anthropic", { method:"POST", headers:{"Content-Type":"application/json"},
                              body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:3000, stream:true,
                                system:`You are a master negotiation strategist. Create a negotiation playbook for a specific deal.

CRITICAL PRICING RULE — BREACH = TOTAL REWRITE:
1. Use ONLY the deal size provided by the rep in the user message. NEVER invent specific dollar figures, contract values, multi-year totals, or pricing that was not provided.
2. openingPosition: Express as a specific % premium over the provided deal size (e.g. "Open at 20% above the provided $400K anchor = $480K annual") OR qualitatively ("Open with premium positioning tied to proven ROI"). If no deal size provided, output EXACTLY this placeholder: "[DEAL SIZE — SET BY REP BEFORE USING THIS PLAYBOOK]".
3. concessionFramework value fields: Express as % of deal size or qualitative impact ("worth 10-15% of annual contract value", "equivalent to one quarter of implementation cost"). NEVER invented dollar amounts.
...
CRITICAL NEGOTIATION RULES (non-negotiable):
- deliveryLine: NEVER invent customer peer benchmarks ("we typically see implementations in your tier delivering...", "customers at your scale have seen...", "companies like yours usually..."). The deliveryLine must be rooted in THIS deal's stated pain and the provided deal size — not invented peer comparisons.
- NEVER cite named customers (e.g. ServiceNow, Databricks, JPMorgan) in deliveryLine, rationale, concession value fields, or counter-tactics. Use [REP TO FILL - INSERT CUSTOMER PROOF POINT] if a proof point would strengthen the line.
- FORBIDDEN phrases throughout: "we typically see", "customers at your scale", "similar deals in this segment", "companies like yours", "our customers usually", "peer organizations have".
- NEVER use a specific vendor name for the SELLER. Refer to "our platform" or "we". Never name "Workiva", "SAP", "Oracle", "Salesforce" etc. as the selling vendor in any pricing, anchor, or counter-tactic field.
